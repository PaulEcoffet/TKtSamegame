...
class PartiePerso(Frame):
    ...
    def __init__(self,menuFrame):
        ...
        self.slider_nb_line = Scale(new,variable = self.nb_colors,orient= HORIZONTAL,from_=2,
                                    to=len(SameGame.possible_colors))
        self.slider_nb_line.grid(row=3,column=1,sticky=W+N+S+E)

        new_game_button = Button(new,text="NOUVELLE PARTIE",command=self.new_game)
